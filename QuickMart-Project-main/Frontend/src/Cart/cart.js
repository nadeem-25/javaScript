...
async function loadPromoCodesFromBackend() {
  try {
    const response = await fetch("https://javascript-b891.onrender.com/promoCodes");
...
async function loadCartFromBackend() {
  ...
  try {
    const response = await fetch("https://javascript-b891.onrender.com/carts");
...
async function saveCartToBackend() {
  ...
  try {
    ...
    if (userCart) {
      await fetch(`https://javascript-b891.onrender.com/carts/${userCart.id}`, {
        method: "PUT",
        headers: {
...
if (checkoutBtn) {
  checkoutBtn.addEventListener("click", function () {
    ...
    const user = getUserFromLocalStorage();
    if (user) {
      fetch("https://javascript-b891.onrender.com/carts")
        .then((res) => res.json())
        .then((carts) => {
          const userCart = carts.find((c) => c.userId === user.id);
          if (userCart) {
            fetch(`https://javascript-b891.onrender.com/carts/${userCart.id}`, {
              method: "DELETE",
            });
          }
        });
